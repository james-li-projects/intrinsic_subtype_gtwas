...
########################################
# Set directory path
dir_path <- "/gpfs/data/huo-lab/BCAC/james.li/XANCESTRY_GWAS_TWAS/output/GWAS/processed_xancestry_meta_sumstats"
# List all TSV files
...
het_p <- fread("/gpfs/data/huo-lab/BCAC/james.li/XANCESTRY_GWAS_TWAS/output/GWAS/lead_variant_output/subtype_heterogeneity/het_p_xancestry.tsv") %>% rename(ID=variant,het_p=p_value)
...
# write output 
write.table(final_df,file="/gpfs/data/huo-lab/BCAC/james.li/XANCESTRY_GWAS_TWAS/output/GWAS/lead_variant_output/subtype_heterogeneity/heterogeneity_table_xancestry.tsv",quote=F,row.names=F,col.names=T,sep="\t")
